...
def generateItemProfiles(R,d,seed,sparkContext,N):
    ...
    """
    # extract items
    items = R.map(lambda x: x[1]).distinct(numPartitions=N)
...
def joinAndPredictAll(R,U,V,N):
    ...
    """
    iRDD = R.map(lambda x: (x[0],(x[1],x[2])))
    gatheredIRDD = iRDD.join(U,numPartitions=N).map(lambda y: (y[1][0][0],(y[0],y[1][0][1],y[1][1]))) # (i,((j,r),u)) -> (j,(i,r,u))
    ans = gatheredIRDD,join(V,numPartitions=N).map(lambda y: (y[1][0][0],y[0],y[1][0][1],y[1][0][2],y[1][1])) # (j,((i,r,u),v)) -> (i,j,r,u,v)
    return ans.map(lambda z: (z[0],z[1],pred_diff(z[2],z[3],z[4]),z[3],z[4])) # (i, j, r, u, v) -> (i, j, pred_diff(r, u, v), u, v)
...
def SE(joinedRDD):
    ...
    """
    return joinedRDD.map(lambda x: x[2]*x[2]).reduce(add)
...
def normSqRDD(profileRDD,param):
	...
	The return value is:
        λ Σ_i ||ui||_2^2 	
    """
    return param * profile.map(lambda x: np.dot(x[1],x[1])).reduce(add)
...
def adaptU(joinedRDD,gamma,lam,N):
    ...
    """
    return joinedRDD.map(lambda x: (x[0], (x[3], gradient_u(x[2], x[3], x[4])))) \
        .reduceByKey(gradient_sum,N) \
        .mapValues(lambda y: y[0] - gamma * (y[1] + 2 * lam * y[0]))	  
...
def adaptV(joinedRDD,gamma,mu,N):
    ...
    """
    return joinedRDD.map(lambda x: (x[1], (x[4], gradient_v(x[2], x[3], x[4])))) \
        .reduceByKey(gradient_sum, numPartitions=N) \
        .mapValues(lambda y: y[0] - gamma * (y[1] + 2 * mu * y[0]))
